  ...
  Object.keys(langMap)
    ...
    .forEach((lang) => {
      ...
      label.innerHTML =
        ...
        lang +
        '" checked><span class="lang-name">' +
        lang +
        '</span><span class="lang-count">(' +
        langMap[lang] +
        "首)</span>";
...
function updateModeAvailability() {
  ...
  const easyRadio = document.querySelector(
    'input[name="guess-mode"][value="easy"]',
  );
  const mediumRadio = document.querySelector(
    'input[name="guess-mode"][value="medium"]',
  );
  const hardRadio = document.querySelector(
    'input[name="guess-mode"][value="hard"]',
  );
...
function loadQuestion() {
  ...
  const usedTitles = new Set([correctTitle]);
  const wrongOptions = [];

  let distractorPool;
  if (correct.lang === "日語" || correct.lang === "英語") {
    distractorPool = allSongs.filter(
      (s) =>
        s.lang === correct.lang &&
        s.title !== correctTitle &&
        s.id !== correct.id,
    );
  } else {
    distractorPool = allSongs.filter(
  ...
  options.forEach((song) => {
    ...
    btn.innerHTML = '<span class="option-title">' + esc(song.title) + "</span>";
    btn.dataset.songId = song.id;
    btn.addEventListener("click", () => handleAnswer(btn, song.id, correct));
    el.appendChild(btn);
  });
...
async function generateGuessCard() {
  ...
  ctx.fillText("SANDY LAM SONG GUESS CHALLENGE", w / 2, 40);
  ctx.fillStyle = "#201E1F";
  ctx.font = "700 26px 'Noto Serif TC',serif";
  ctx.fillText("猜歌挑戰成績", w / 2, 80);
  ...
  ctx.lineTo(w - 60, 450);
  ctx.stroke();
  const url = "https://sandylam.netlify.app/guess.html";
  ctx.textAlign = "left";
  ctx.fillStyle = "#201E1F";
  ctx.font = "600 13px 'Noto Sans TC',sans-serif";
